      ...
      _onmessage: function(m) {
          if ('connect'!=m.data){
             ...
             if(result.username!= undefined && result.order=='1'){
             	$('A').className='';
             	$('playa').innerHTML='<b>��ң�'+result.username+',�÷֣�score��'+result.score+',status:'+status+'</b>';
             }
             if(result.username!= undefined && result.order=='2'){
             	$('B').className='';
             	$('playb').innerHTML='<b> ��ң�'+result.username+',�÷֣�score��'+result.score+'--status:'+status+'</b>';
             }
             if(result.username!= undefined && result.order=='3'){
             	$('C').className='';
             	$('playc').innerHTML='<b> ��ң�'+result.username+',�÷֣�score��'+result.score+'--status:'+status+'</b>';
             }
           }
        },
